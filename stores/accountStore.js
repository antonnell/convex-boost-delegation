...
class Store {
        ...
        if (isAuthorized) {
          injected
            ...
            .then(a => {
              ...
              this.emitter.emit(CONNECTION_CONNECTED);
              this.emitter.emit(CONFIGURE_RETURNED);
              this.dispatcher.dispatch({ type: CONFIGURE_BOOST });
            })
            .catch(e => {
  ...
  updateAccount = () => {
    ...
    const res = window.ethereum.on("accountsChanged", function(accounts) {
      ...
      that.emitter.emit(CONNECTION_CONNECTED);
      that.emitter.emit(CONFIGURE_RETURNED);

      that.dispatcher.dispatch({ type: CONFIGURE_BOOST });
    });
  };
